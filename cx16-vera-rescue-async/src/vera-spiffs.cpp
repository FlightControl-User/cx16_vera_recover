 ...
 * @brief A collection of SPIFFS routines for SPIFFS file system I/O.
 * These routines are mostly used as an example.
 * @version 0.1
 * @date 2023-10-21
 *
...
void vera_spiffs_list_dir(fs::FS &fs, const char *dirname, uint8_t levels)
{
    ...
    while (file)
    {
        if (file.isDirectory())
        {
            ...
            if (levels)
            {
                vera_spiffs_list_dir(fs, file.name(), levels - 1);
            }
        }
        else
        {
